...
namespace Hook
{
	...
	inline T ReadMemory(AT addr)
	{
		DWORD m_OldVirtualProtect[2];

		VirtualProtect(reinterpret_cast<void*>(addr), sizeof(T), PAGE_EXECUTE_READWRITE, &m_OldVirtualProtect[0]);
		T result =  *reinterpret_cast<T*>(addr);
		VirtualProtect(reinterpret_cast<void*>(addr), sizeof(T), m_OldVirtualProtect[0], &m_OldVirtualProtect[1]);
		return result;
	}
